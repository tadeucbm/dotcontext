...
function registerSkillCommands(parent: Command, hidden = false): Command {
  ...
  skillCommand
    ...
    .action(async (repoPath: string, options: any) => {
      try {
        const { createSkillRegistry } = await import('../harness/domain/workflow/skills');
  ...
  skillCommand
    ...
    .action(async (repoPath: string, options: any) => {
      try {
        const { SkillExportService } = await import('../harness/application/exchange/export/skillExportService');
...
export async function runDotcontextCli(): Promise<void> {
...
export function runDotcontextCliFromProcess(): void {
  runDotcontextCli().catch(error => {
